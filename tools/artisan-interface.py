...
process = None
control = True # Hottop under control?
...
xCONTROL = True # False: just logging; True: logging+control
xBT = None
xET = None
xHEATER = None
xFAN = None
...
def doWork(interval, comport, baudrate, bytesize, parity, stopbits, timeout,
    ...
    while True:
        ...
        print "Config", BT, ET, HEATER, FAN, MAIN_FAN, SOLENOID, DRUM_MOTOR, COOLING_MOTOR
        if BT != -1:
            if aBT.value == -1:
                aBT.value = float(BT)
            else:
...
def main():
    ...
    startHottop(0.6, USB_PORT, 115200, 8, "N", 1, 1)
    time.sleep(5)
    takeHottopControl()
